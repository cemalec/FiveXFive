...
import { themes, AppTheme, ThemeName } from "../theme";
...
export default function SettingsScreen({
  ...
  function fromDisplay(value: number): number {
    ...
    if (workoutState.unit === "kg" && workoutState.storageUnit === "lbs")
  ...
  const [increments, setIncrements] = useState<Record<ExerciseKey, string>>(
    Object.fromEntries(
      EXERCISES.map(({ key }) => [
        key,
        toDisplayIncrement(key, workoutState.increments[key]),
      ]),
    ) as Record<ExerciseKey, string>,
  );
  ...
  useEffect(() => {
    ...
    workoutState.unit,
    workoutState.storageUnit,
    workoutState.weights,
    workoutState.increments,
    workoutState.customWarmupPercentages,
  ]);
  ...
  function buildDraftState(): WorkoutState | null {
    ...
    if (
      parsedWarmupPercentages.length === 0 ||
      parsedWarmupPercentages.some((value) => value <= 0 || value >= 100)
    ) {
      Alert.alert(
        "Invalid warmups",
        "Warmup percentages must be comma-separated numbers between 0 and 100.",
      );
      return null;
    }
    ...
    for (const { key } of EXERCISES) {
      ...
      if (isNaN(w) || w <= 0 || isNaN(inc) || inc <= 0) {
        Alert.alert(
          "Invalid value",
          "All weights and increments must be positive numbers.",
        );
        return null;
      }
  ...
  async function handleResetWarmupPercentages() {
    ...
    const newState: WorkoutState = {
      ...workoutState,
      customWarmupPercentages: defaults,
    };
    await saveWorkoutState(newState);
    onSave(newState);
  }
  ...
  async function handleRound(targetUnit: Unit) {
    ...
    const stepText =
      targetUnit === "kg"
        ? "2.5 kg (5 kg for deadlift)"
        : "5 lb (10 lb for deadlift)";
  ...
  async function handleResetIncrements() {
    const defaultIncrements = standardIncrementsForUnit(
      workoutState.storageUnit,
    );
    const newState: WorkoutState = {
      ...workoutState,
  ...
  function handleConvert(targetUnit: Unit) {
    const label =
      targetUnit === "kg" ? "kg (nearest 2.5 kg)" : "lbs (nearest 5 lb)";
    Alert.alert(
      "Convert weights?",
    ...
    return (
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.header}>
          <TouchableOpacity
            onPress={() => setShowIncrementSettings(false)}
        ...
        >
          <Text style={styles.sectionHeader}>
            Increment Per Workout ({unitLabel})
          </Text>
          <View style={styles.card}>
            {EXERCISES.map(({ key, label }, idx) => (
              <View
                ...
                style={[
                  styles.row,
                  idx === EXERCISES.length - 1 && styles.rowLast,
                ]}
              >
          ...
          <TouchableOpacity
            ...
            onPress={handleResetIncrements}
          >
            <Text style={styles.secondaryButtonText}>
              Reset To Standard Increments
            </Text>
          </TouchableOpacity>
  ...
  if (showWarmupSettings) {
    return (
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.header}>
          <TouchableOpacity
            onPress={() => setShowWarmupSettings(false)}
            style={styles.headerSide}
          >
            <Text style={styles.backArrow}>‹</Text>
          </TouchableOpacity>
        ...
        >
          ...
          <View style={styles.card}>
            <Text style={styles.convertNote}>
              Enter comma-separated percentages for the percentage warmup mode.
              Example: 45, 65, 85
            </Text>
          ...
          <TouchableOpacity
            style={styles.secondaryButton}
            onPress={handleResetWarmupPercentages}
          >
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity
          ...
          style={[styles.headerSide, styles.unitToggle]}
        >
          <Text style={styles.unitToggleText}>
            {workoutState.unit === "lbs" ? "kg" : "lbs"}
          </Text>
        </TouchableOpacity>
      </View>
      ...
      >
        ...
        <View style={styles.card}>
          <Text style={styles.convertNote}>
            Choose between empty bar plus 3 even steps, or empty bar plus
            percentage-based warmups.
          </Text>
          <TouchableOpacity
            style={[
              styles.roundButton,
              workoutState.warmupMode === "interpolate" &&
          ...
          >
            <Text
              style={[
                ...
                workoutState.warmupMode === "interpolate" &&
                  styles.selectedActionButtonText,
              ]}
            >
          ...
          <TouchableOpacity
            style={[
              styles.roundButton,
              workoutState.warmupMode === "percentages" &&
                styles.selectedActionButton,
              styles.roundButtonLast,
            ]}
          ...
          <TouchableOpacity
            style={styles.secondaryButton}
            onPress={() => setShowWarmupSettings(true)}
          >
            <Text style={styles.secondaryButtonText}>
        ...
        <View style={styles.card}>
          <Text style={styles.convertNote}>
            ...
            {workoutState.unit === "kg" ? "2.5 kg" : "5 lb"} for most lifts and
            doubles for deadlift.
          </Text>
          <TouchableOpacity
            style={styles.roundButton}
            onPress={() => setShowIncrementSettings(true)}
          >
            <Text style={styles.roundButtonText}>
              Advanced Increment Settings
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.roundButton, styles.roundButtonLast]}
            onPress={handleResetIncrements}
          >
        ...
        <View style={styles.card}>
          <Text style={styles.convertNote}>
            Rounds all current weights and increments to standard plate jumps
            without changing the stored unit system.
          </Text>
          <TouchableOpacity
            style={styles.roundButton}
            onPress={() => handleRound("kg")}
          >
            <Text style={styles.roundButtonText}>
              Round Everything To Kg Increments
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.roundButton, styles.roundButtonLast]}
            onPress={() => handleRound("lbs")}
          >
            <Text style={styles.roundButtonText}>
              Round Everything To Lb Increments
            </Text>
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.card}>
          <Text style={styles.convertNote}>
            Optional. When enabled, each completed workout updates a CSV backup
            stored on this device. On iOS, that backup can be copied out through
            the Files app while the app remains installed.
          </Text>
        ...
        <View style={styles.card}>
          <Text style={styles.convertNote}>
            Rounds all stored weights to clean plate increments in the chosen
            unit and resets increments to standard values. This overwrites your
            current data.
          </Text>
          <TouchableOpacity
            style={[
              styles.convertRow,
              workoutState.storageUnit === "kg" && styles.convertRowDisabled,
            ]}
          ...
          >
            <Text
              style={[
                styles.convertText,
                workoutState.storageUnit === "kg" && styles.convertTextDisabled,
              ]}
            >
          ...
          <TouchableOpacity
            style={[
              styles.convertRow,
              styles.convertRowLast,
              workoutState.storageUnit === "lbs" && styles.convertRowDisabled,
            ]}
          ...
          >
            <Text
              style={[
                styles.convertText,
                workoutState.storageUnit === "lbs" &&
                  styles.convertTextDisabled,
              ]}
            >
